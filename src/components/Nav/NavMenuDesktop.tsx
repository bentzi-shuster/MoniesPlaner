      ...
      {navs.map((nav,index)=>{
        ...
        <Tabs.Trigger
         ...
         data-state={linkpath===nav.Link?"active":"inactive"}
          className="bg-white px-5 h-[45px] flex-1 flex items-center justify-center text-[15px] leading-none text-mauve11 select-none  hover:text-green-900 data-[state=active]:text-green-700 data-[state=active]:shadow-[inset_0_-1px_0_0,0_1px_0_0] data-[state=active]:shadow-current data-[state=active]:focus:relative data-[state=active]:focus:shadow-[0_0_0_2px] data-[state=active]:focus:shadow-black outline-none cursor-default"
          value={nav.Link}
          key={index}
        >
          <Link   onClick={(e)=>{
